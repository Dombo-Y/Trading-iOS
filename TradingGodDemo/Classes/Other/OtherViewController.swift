...
class OtherViewController: BaseViewController , TGWebSocketDelegate{
    ...
    var beforePrice:CGFloat = 0.0
    
    lazy var dataView: TradesDataView = {
        let v = TradesDataView.init(frame: CGRect(x: 0, y: 380, width: kSCREEN_WIDTH, height: 200))
...
//            print("trades ====== \(String(describing: dic))");
            let mmm = TGTradesModel.yy_model(with: dic as! [AnyHashable : Any])
            if beforePrice > 0 {
                mmm?.bf_px = beforePrice
            }
            beforePrice =  mmm?.bf_px ?? 0
...
//                let px_f:CGFloat = mmm?.px.StringToCGFloat() ?? 0
                ...
                mmm?.buyVolume = buyVolume
                mmm?.sellVolume = sellVolume
                print(mmm!)
